...
function OnChat( fromPlayer, toPlayer, text, eTargetType )
    ...
    TruncateString( controlTable.String, 200, Players[fromPlayer]:GetCivilizationShortDescription() );
    ...
    elseif( eTargetType == ChatTargetTypes.CHATTARGET_PLAYER ) then
        ...
        if( toPlayer == g_iLocalPlayer ) then
            ...
            TruncateString( controlTable.String, 200, Players[toPlayer]:GetCivilizationShortDescription() );
...
function OnChatTarget( iTeam, iPlayer )
    ...
    if( iTeam ~= -1 ) then
        ...
        if( iPlayer ~= -1 ) then
			TruncateString( Controls.LengthTest, Controls.ChatPull:GetSizeX(), Locale.ConvertTextKey("TXT_KEY_DIPLO_TO_PLAYER", Players[ iPlayer ]:GetCivilizationShortDescription()));
            Controls.ChatPull:GetButton():SetText( Controls.LengthTest:GetText() );
        else
...
function PopulateChatPull()
    ...
    for iPlayer = 0, GameDefines.MAX_PLAYERS do
        ...
        if( iPlayer ~= g_iLocalPlayer and pPlayer ~= nil and pPlayer:IsHuman() ) then
			...
			TruncateString( textControl, Controls.ChatPull:GetSizeX()-20, Locale.ConvertTextKey("TXT_KEY_DIPLO_TO_PLAYER", pPlayer:GetCivilizationShortDescription()));
        end
    end
