		...
		elseif (iLeadingNumPlayersOnTeam > 1) then
			...
			if (iLeadingPlayer == Game.GetActivePlayer()) then
				...
				if (pOtherPlayer:GetNickName() ~= "" and pOtherPlayer:IsHuman()) then
					Controls.DominationLabel:SetText(Locale.ConvertTextKey("TXT_KEY_VP_DIPLO_CAPITALS_PLAYER_LEADING", pOtherPlayer:GetCivilizationShortDescription(), iLeadingNumCapitals));
	...
	if (victoryId ~= nil and PreGame.IsVictory(victoryId))then
		...
		if(Game.GetVictory() == victoryId) then
			...
			if(Game:IsNetworkMultiPlayer() and leader:GetNickName() ~= "" and leader:IsHuman() ) then
				playerName = leader:GetCivilizationShortDescription();
			else
	...
	if (pTeam:IsHasMet(Game.GetActiveTeam()) or Game:IsNetworkMultiPlayer()) then
		...
		if(controlTableTT ~= nil) then
			if(Game:IsNetworkMultiPlayer() and pPlayer:GetNickName() ~= "") then
				controlTableTT:SetToolTipString(pPlayer:GetCivilizationShortDescription());
		...
		if (iPlayer == iDominatingPlayer) then
			if (pTeam:IsHasMet(Game.GetActiveTeam())) then
				...
				if (iPlayer == Game.GetActivePlayer()) then
					...
					if (pPlayer:GetNickName() ~= "" and Game:IsNetworkMultiPlayer()) then
						controlTableTT:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_VP_DIPLO_TT_SOMEONE_CONTROLS_THEIR_CAPITAL", pPlayer:GetCivilizationShortDescription(), pOriginalCapital:GetNameKey()));
					else
			...
			if (pTeam:IsHasMet(Game.GetActiveTeam())) then
				...
				if (pDominatingTeam:IsHasMet(Game.GetActiveTeam())) then
					if (iPlayer == Game.GetActivePlayer()) then
						if (pDominatingPlayer:GetNickName() ~= "" and Game:IsNetworkMultiPlayer()) then
							controlTableTT:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_VP_DIPLO_TT_OTHER_PLAYER_CONTROLS_YOUR_CAPITAL", pDominatingPlayer:GetCivilizationShortDescription(), pOriginalCapital:GetNameKey()));
						else
						...
						if (iDominatingPlayer == Game.GetActivePlayer()) then
							...
							if (pDominatingPlayer:GetNickName() ~= "" and Game:IsNetworkMultiPlayer()) then
								controlTableTT:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_VP_DIPLO_TT_OTHER_PLAYER_CONTROLS_OTHER_PLAYER_CAPITAL", pDominatingPlayer:GetCivilizationShortDescription(), pOriginalCapital:GetNameKey(), pPlayer:GetCivilizationShortDescriptionKey()));
							else
				...
				if (pDominatingTeam:IsHasMet(Game.GetActiveTeam())) then
					if (pDominatingPlayer:GetNickName() ~= "" and Game:IsNetworkMultiPlayer()) then
						controlTableTT:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_VP_DIPLO_TT_OTHER_PLAYER_CONTROLS_UNMET_PLAYER_CAPITAL", pDominatingPlayer:GetCivilizationShortDescription()));
					else
